...
SELECT l.mottatt_dato,
       ...
       l.innsending_id,
       l.type,
       (SELECT i.id from soknad_ettersending s JOIN innsending_ny i ON i.ekstern_referanse=s.innsending_soknad_ref WHERE s.innsending_ettersending_ref = l.innsending_id ),
       l.journalpost_id
FROM Logg l ORDER BY l.mottatt_dato;
